...
import { AlertTriangle, CalendarDays, Check, ChevronRight, Clock3, Download, History, LoaderCircle, Pencil, Plus, Search, ShieldCheck, Timer, Trash2, UserCheck, Users, X } from "lucide-react";
...
import { CONTRACT_LABELS, dailyTargetMinutes, overtimeBalanceHours, weeklyMinutesForContract } from "@/lib/contract";
...
export function AdminTimeTrackingPage() {
  ...
  // Hinweis für die Vertretung (sieht der Springer im Vertretungskalender).
  ...
  // Zeilenfilter der Monatsübersicht: Klick auf eine Zeile filtert die
  // Detail-Tabellen (Prüfbedarf/Freigabe-Feed) auf diese Person.
  // Erneuter Klick auf dieselbe Zeile (oder der Chip) hebt den Filter auf.
  ...
  function openEditRequest(requestId: string) {
    ...
    setEditNote(request.reviewer_note ?? "");
  }
  ...
  async function saveEditRequest() {
    ...
    try {
      const res = await offlineFetch(`/api/time-tracking/requests/${editTarget.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status: editTarget.status, substitute_id: editSubstitute || null, reviewer_note: editNote }),
      });
      const body = await res.json().catch(() => ({}));
  ...
  async function createAbsence() {
    ...
    try {
      const res = await offlineFetch("/api/time-tracking/requests", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id: createEmployeeId, type: createType, start_date: createStart, end_date: createEnd, substitute_id: createSubstitute || null, reviewer_note: createNote, status: "approved" }),
      });
      const body = await res.json().catch(() => ({}));
   ...
   * Vertrags-Default 10 h) hochgerechnet auf die Tagesanzahl des gewählten
   * Monats. null für Nicht-Minijobber.
   */
  function miniJobLimitMinutes(employee: Employee): number | null {
    if (employee.contract_type !== "mini_job") return null;
    const [year, monthPart] = month.split("-").map(Number);
    if (!year || !monthPart || monthPart < 1 || monthPart > 12) return null;
    const daysInMonth = new Date(year, monthPart, 0).getDate();
    const weekly = weeklyMinutesForContract("mini_job", employee.weekly_target_hours ?? null);
    return Math.round((weekly * daysInMonth) / 7);
  }

  // Ausgewählte Person + die damit gefilterten Detail-Listen
  // (Prüfbedarf & Freigabe-Feed unterhalb der Monatsübersicht).
  const selectedEmployeeName = selectedUserId
    ? overview?.employees.find((employee) => employee.id === selectedUserId)?.name ?? null
    : null;
  const filteredReviewEntries = selectedUserId
    ? reviewEntries.filter((entry) => entry.user_id === selectedUserId)
    : reviewEntries;
  const filteredPendingEntries = selectedUserId
    ? pendingEntries.filter((entry) => entry.user_id === selectedUserId)
    : pendingEntries;
  ...
  return (
    <div className="container py-6 sm:py-10">
      ...
      {loading && !overview ? <div className="mt-8 flex items-center gap-2 text-sm text-muted-foreground"><LoaderCircle className="h-4 w-4 animate-spin" /> Übersicht wird geladen…</div> : overview ? <>
        ...
        {/* Aktiver Zeilenfilter der Monatsübersicht – per Chip abwählbar. */}
        {selectedUserId && selectedEmployeeName && (
          <div className="mt-4 flex items-center gap-2">
            <span className="text-sm text-muted-foreground">Detail-Filter:</span>
            <button
              type="button"
              onClick={() => setSelectedUserId(null)}
              title="Filter entfernen (alle anzeigen)"
              className="inline-flex items-center gap-1.5 rounded-full border bg-primary/10 px-3 py-1 text-sm font-medium text-primary transition-colors hover:bg-primary/20"
            >
      ...
      {pendingRequests.map((request) => <div key={request.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3"><div><div className="flex flex-wrap items-center gap-1.5 text-sm font-medium"><CalendarDays className="h-3.5 w-3.5 text-muted-foreground" />{REQUEST_TYPE[request.type] ?? request.type} · {request.profiles?.name ?? request.user_id}<Badge variant="warning">Ausstehend</Badge></div><p className="text-xs text-muted-foreground">{new Date(`${request.start_date}T00:00:00`).toLocaleDateString("de-DE")} – {new Date(`${request.end_date}T00:00:00`).toLocaleDateString("de-DE")}</p>{request.employee_note && <p className="mt-1 text-xs text-muted-foreground">Notiz: {request.employee_note}</p>}</div><div className="flex gap-2"><Button size="sm" onClick={() => openApproval(request)} disabled={saving === request.id}>Genehmigen</Button><Button size="sm" variant="outline" onClick={() => void reviewRequest(request, "rejected", request.substitute_id ?? undefined, request.reviewer_note ?? undefined)} disabled={saving === request.id}>Ablehnen</Button></div></div>)}{filteredPendingEntries.length === 0 && (selectedUserId ? <p className="text-sm text-muted-foreground">Keine offenen Freigaben für {selectedEmployeeName ?? "die gewählte Person"}.</p> : pendingEntries.length === 0 && pendingRequests.length === 0 && <p className="text-sm text-muted-foreground">Keine offenen Freigaben.</p>)}</CardContent></Card>
        ...
        <Dialog open={editTarget !== null} onOpenChange={(open) => { if (!open) setEditTarget(null); }}>
          <DialogContent className="sm:max-w-md">
            ...
            {editTarget && <div className="space-y-4"><div className="space-y-2"><Label>Vertretung durch (optional)</Label><Select value={editSubstitute || "none"} onValueChange={(value) => setEditSubstitute(value === "none" ? "" : value)}><SelectTrigger><SelectValue placeholder="Keine Vertretung" /></SelectTrigger><SelectContent><SelectItem value="none">Keine Vertretung</SelectItem>{[...overview.employees].filter((employee) => employee.id !== editTarget.user_id).sort((a, b) => Number(b.role === "substitute") - Number(a.role === "substitute") || a.name.localeCompare(b.name, "de")).map((employee) => <SelectItem key={employee.id} value={employee.id}>{employee.name} · {ROLE_LABELS[employee.role] ?? employee.role}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label htmlFor="edit-substitute-note">Hinweis für die Vertretung (optional)</Label><Input id="edit-substitute-note" value={editNote} onChange={(event) => setEditNote(event.target.value)} maxLength={1000} placeholder="z. B. besondere Absprachen" /><p className="text-xs text-muted-foreground">Wird dem Springer in seinem Vertretungskalender angezeigt.</p></div><DialogFooter><Button variant="outline" onClick={() => setEditTarget(null)}>Abbrechen</Button><Button onClick={() => void saveEditRequest()} disabled={editSaving}>{editSaving ? "Wird gespeichert…" : "Speichern"}</Button></DialogFooter></div>}
          </DialogContent>
        </Dialog>
